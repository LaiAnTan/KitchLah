...
def get_expirty_alert():
    ...
    try:
        ...
        # Validate required ML model URL
        if 'ml_model_url' not in data:
            return jsonify({'error': 'ml_model_url is required'}), 400
        
        ml_model_url = data['ml_model_url']
        
        # Step 1: Call ML model to get 7-day ingredient predictions
        try:
            ml_response = requests.post(ml_model_url, json={
                'prediction_days': 7,
            }, timeout=30)
        ...
        # Get all ingredients with their active restocks
        from utils.database import db
        pipeline = [
            {
                '$lookup': {
