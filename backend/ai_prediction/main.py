...
csv_path = os.path.join(BASE_DIR, "data", "kitchen_orders_synthetic.csv")
model_path = os.path.join(BASE_DIR, "data", "kitchen_nn.pth")

load_dotenv()
uri = os.getenv('MONGODB_URI')
client = MongoClient(uri)
db = client.kitchlah_db
surge_detection = db.surge_detection
...
def run():
    ...
    for _, row in daily_totals.iterrows():
        ...
        for k, v in ingredients.items():
            unit = ingredients_db.find_one({ '_id': k })['unit']
            ingredients_kg[k] = v/1000 if unit != "grams" else v
